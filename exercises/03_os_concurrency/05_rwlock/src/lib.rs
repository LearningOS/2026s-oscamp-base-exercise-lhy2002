...
impl<T> RwLock<T> {
    ...
    pub fn read(&self) -> RwLockReadGuard<'_, T> {
        ...
        loop{
			...
			if self.state.compare_exchange(state, state + 1, Ordering::Release, Ordering::Acquire).is_ok() {
    ...
    pub fn write(&self) -> RwLockWriteGuard<'_, T> {
        ...
        loop{
			...
			if self.state.compare_exchange(WRITER_WAITING, WRITER_HOLDING, Ordering::Release, Ordering::Acquire).is_ok() {
				return RwLockWriteGuard { lock: self };
			}
			if self.state.compare_exchange(0, WRITER_HOLDING, Ordering::Release, Ordering::Acquire).is_ok() {
				return RwLockWriteGuard { lock: self };
			}
