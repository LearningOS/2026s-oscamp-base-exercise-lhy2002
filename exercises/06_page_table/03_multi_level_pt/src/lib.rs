...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let node = self.nodes.get_mut(&curr_ppn).unwrap();
        node.entries[vpn0] = ((pa >> 12) << PPN_SHIFT) | flags | PTE_V;
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            let vpn = Self::extract_vpn(va, level);
            let pte = match self.nodes.get(&curr_ppn) {
            ...
            if (pte & (PTE_R | PTE_W | PTE_X)) != 0 {
                ...
                let page_offset_mask = (1 << (12 + level * 9)) - 1;
                let paddr = (ppn << 12) | (va & page_offset_mask);
                
                return TranslateResult::Ok(paddr);
            }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte2 = self.nodes.get(&curr_ppn).unwrap().entries[vpn2];
        
        if (pte2 & PTE_V) == 0 {
            let next_ppn = self.alloc_node();
            self.nodes.get_mut(&curr_ppn).unwrap().entries[vpn2] = (next_ppn << PPN_SHIFT) | PTE_V;
            curr_ppn = next_ppn;
        } else {
            curr_ppn = pte2 >> PPN_SHIFT;
        }
