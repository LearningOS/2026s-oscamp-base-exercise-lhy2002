...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    ...
    let sum_a = thread::spawn(move || a.iter().sum::<i32>());
    let sum_b = thread::spawn(move || b.iter().sum::<i32>());
	let (sum_a, sum_b) = (sum_a.join().unwrap(), sum_b.join().unwrap());
	(sum_a, sum_b)
}
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    ...
    let (sum_a, sum_b) = thread::scope(|scope| {
        let h1 = scope.spawn(|| a.iter().sum::<i32>());
        let h2 = scope.spawn(|| b.iter().sum::<i32>());
		(h1.join().unwrap(), h2.join().unwrap())
    });
    (sum_a, sum_b)
}
